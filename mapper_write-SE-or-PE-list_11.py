...
class mapper_call:
    def __init__(self,f1,fout1,isSE,dirout1,concat,f_factors):
        ...
        self.fout2 = self.fout2.replace("_err.log", "")
        ...
        for i,group1 in enumerate(self.g.keys()):
            self.grp[group1]=[[],[]]
            for j,pair1 in enumerate(sorted (self.g[group1].keys())):
        ...
        if not (self.concat): self.out1.write( '# script template created by %s from table %s\n'%(os.path.basename(__file__),self.f1) )
        for nf in notFile:
    ...
    def write_bash_script(self,delim1=','):
        ...
        for i,g in enumerate(sorted(self.grp.keys())): # groups
            if self.pairing=='PE':
                if(len(self.grp[g])==2) and (len(self.grp[g][0])>0) and (len(self.grp[g][1])>0) and (len(self.grp[g][0])==len(self.grp[g][1])):
                    ...
                    if not (self.concat): self.out1.write( self.grp[g] +"\n")
        ...
        if not (self.concat): self.out1.write( '\n# files:' + "\n" )
        if(self.concat): self.out2.write("dirout_cat=%s\n\n"%(o(self.dirout1)))
        if not (self.concat): self.out1.write( "function assert_ { rc=$?; if [[ $rc != 0 ]]; then echo 'exit !!!!'; exit $rc; fi }" +"\n")
        ...
        for i,c in enumerate(cmd):
            if not (self.concat): self.out1.write( c +"\n")
        ...
        if not (self.concat): self.out1.write( "\n#run:" +"\n")
        if self.pairing == 'PE':
            if not (self.concat): self.out1.write( "#for i in ${!title[@]}; do\n#\techo $i\n#\techo ${title[i]}\n#\techo ${for1[i]}\n#\techo ${rev1[i]}\n#\techo '----------------'\n#done" +"\n")
            #if(self.concat):
